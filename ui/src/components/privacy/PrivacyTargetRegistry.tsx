...
export function PrivacyTargetRegistry({
  ...
  const filteredTargets = useMemo(
    () =>
      targets.targets.filter(
        (target) => category === "all" || target.category === category,
      ),
  ...
  return (
    <section className={`rounded-xl border ${border} ${panel} p-5`}>
      ...
      <div
        className={`mt-4 rounded-lg border px-3 py-2 text-[10px] font-mono uppercase tracking-widest ${targets.selectedCount > 0 ? okClass : warnClass}`}
      >
        {statusText}
      </div>
    </section>
