...
def render_review_sql(plan: OwnerPlan) -> str:
    ...
    for statement in ownership_statements(plan):
...
def ownership_statements(plan: OwnerPlan) -> list[str]:
    statements: list[str] = []
    for obj in plan.objects:
        if obj.kind == "role" or obj.owner != "jarvisbrain":
            continue
        if obj.kind == "database":
            statements.append(
                f"ALTER DATABASE {obj.identity} OWNER TO {plan.owner_role};"
            )
        elif obj.kind == "schema":
            statements.append(
                f"ALTER SCHEMA {obj.identity} OWNER TO {plan.owner_role};"
            )
        elif obj.kind == "extension":
            statements.append(
                f"ALTER EXTENSION {obj.identity} OWNER TO {plan.owner_role};"
            )
        elif obj.kind == "relation":
            statements.append(f"ALTER {obj.identity} OWNER TO {plan.owner_role};")
        elif obj.kind == "function":
            statements.append(
                f"ALTER FUNCTION {obj.identity} OWNER TO {plan.owner_role};"
            )
    return statements
