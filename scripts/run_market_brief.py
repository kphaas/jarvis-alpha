#!/usr/bin/env python3
"""Run and persist the scheduled global market brief for Helm."""
...
async def _run() -> dict[str, object]:
    ...
    from brain.services.internet_scout.market_brief import run_market_brief_once

    pool = await _create_pool(_writer_dsn())
    try:
        async with platform_admin_connection(
            source="scheduled",
            audit_actor="market_global_brief",
            pool=pool,
        ) as conn:
            return await run_market_brief_once(conn)
    finally:
        await pool.close()
