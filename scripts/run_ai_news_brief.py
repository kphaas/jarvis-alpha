#!/usr/bin/env python3
"""Run and persist the scheduled AI vendor news brief for Helm."""
...
async def _run() -> dict[str, object]:
    ...
    from brain.services.internet_scout.ai_news_brief import run_ai_news_brief_once

    pool = await _create_pool(_writer_dsn())
    try:
        async with platform_admin_connection(
            source="scheduled",
            audit_actor="ai_news_daily_brief",
            pool=pool,
        ) as conn:
            return await run_ai_news_brief_once(conn)
    finally:
        await pool.close()
