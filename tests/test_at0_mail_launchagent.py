...
HEALTH_PLIST = (
    REPO_ROOT / "launchagents" / "com.jarvis.alpha.at0-mail-health.template.plist"
)
HEALTH_START_SCRIPT = REPO_ROOT / "scripts" / "start_alpha_at0_mail_health.sh"
...
def test_at0_mail_health_launchagent_template_renders() -> None:
    rendered = HEALTH_PLIST.read_text(encoding="utf-8").replace(
        "{{HOME}}", "/Users/test"
    )
    parsed = plistlib.loads(rendered.encode("utf-8"))

    assert parsed["Label"] == "com.jarvis.alpha.at0-mail-health"
    assert parsed["RunAtLoad"] is True
    assert parsed["StartInterval"] == 900
    assert parsed["ProgramArguments"] == [
        "/bin/bash",
        "/Users/test/jarvis-alpha/scripts/start_alpha_at0_mail_health.sh",
    ]


def test_at0_mail_health_start_script_has_valid_bash_syntax() -> None:
    result = subprocess.run(
        ["bash", "-n", str(HEALTH_START_SCRIPT)],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    assert result.returncode == 0, result.stderr
    assert "-m brain.agents.at0_mail_health_watcher" in HEALTH_START_SCRIPT.read_text(
        encoding="utf-8"
    )
...
def test_at0_mail_launchagent_registered_for_brain() -> None:
    ...
    assert SERVICE_NODE_MAP.get("com.jarvis.alpha.at0-mail-health") == "brain"
