...
| AUDIT-3 | `/health` does not probe DB / Ollama / Temporal | P1 | ✅ CLOSED | New `GET /health/ready` (`brain/routes/health.py` + `brain/health_probes/`) probes DB pool, Ollama, Temporal in parallel via `asyncio.gather`; per-check timeout (DB/Ollama 200ms, Temporal 1.0s — cold gRPC connect exception, see TD); HTTP 200 all-pass / 503 any-fail; status-report only (no recovery). Shallow `/health` untouched (backward-compat); route added to JWT `SKIP_PATHS` | Session 2.5 (this PR) |
...
| AUDIT-6 | Loki ingest live, labeling broken + coverage incomplete | P1 | ✅ CLOSED | Closed by Phase 4c (PR #161): added `json_log` parser to `config/observability/brain/fluent-bit.yaml` and attached it to the 3 structured-stdout tail inputs (buddy, executor, watchdog) → `service`/`level` labels now resolve (verified `service`/`service_name` multi-value in Loki, no longer collapsed to `unknown_service`). Format-migration follow-ups tracked separately as TDs #162, #163, #164, #165 (all P3-low + tech-debt): uvicorn-access→JSON, `*_error.log` `timestamp`→`ts`, power_sampler→JSON, emit-name normalization. Fleet extension (Gateway/Sandbox shipper, Endpoint config audit, missing 5 Brain tails) remains Session 3+. See `docs/observability/LABEL_TAXONOMY.md`. | Phase 4c (PR #161); TDs #162-#165 |
...
| AUDIT-8 | `/health/agents` only surfaces 3 of 8+ LaunchAgents | P2 | ✅ CLOSED | `brain/routes/health.py` — `BRAIN_AGENTS` extended from 3 to 11 always-on agents: alpha.brain, buddy, executor, watchdog, fluentbit, loki, temporal.server, temporal.ui, temporal.worker, power.brain, ollama. Scheduled agents (pg_backup, rotate.brain_service, rotate.buddy, school-email) deliberately excluded as not-always-on to avoid false "not running" alarms — tracked as TD-AGENT-CATEGORIZATION (§ below) | PR #159 (Session 2) |
| AUDIT-9 | Rate-limit docstring/constant mismatch | P2 | ✅ CLOSED | `brain/middleware/rate_limit_middleware.py` docstring synced to the `MAX_REQUESTS = 300` constant (was "100 req/min per user_id") | PR #158 (Session 2) |
