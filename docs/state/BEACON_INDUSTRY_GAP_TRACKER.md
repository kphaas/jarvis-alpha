...
| MCP/tool ecosystem | Beacon now has a documented internal tool ecosystem contract, a machine-readable crawler MCP adapter spec, and a disabled-by-default runtime status skeleton that maps discovery to existing Alpha routes without creating a new execution path. | Skeleton-ready | Beacon exposes policy-scoped tool contracts for approved internal agents and optional MCP-facing consumers. |
...
| 18 | MCP adapter | Skeleton-ready | `docs/contracts/beacon_crawler_mcp_adapter.v1.json` defines the thin crawler adapter contract, and `GET /v1/security/mcp/adapters/beacon-crawler` exposes disabled status only; invocation stays deferred until Beacon has a real external MCP client or cross-agent protocol requirement. |
