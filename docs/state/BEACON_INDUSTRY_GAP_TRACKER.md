...
| MCP/tool ecosystem | Beacon now has a documented internal tool ecosystem contract plus a machine-readable crawler MCP adapter spec that maps MCP tools to existing Alpha routes without bypassing auth, Gateway egress, approval, caps, cache, or audit. | Spec-ready | Beacon exposes policy-scoped tool contracts for approved internal agents and optional MCP-facing consumers. |
...
| 18 | MCP adapter | Spec-ready | `docs/contracts/beacon_crawler_mcp_adapter.v1.json` defines the thin crawler adapter contract; runtime activation stays deferred until Beacon has a real external MCP client or cross-agent protocol requirement. |
