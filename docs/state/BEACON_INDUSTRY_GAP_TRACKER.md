...
| Research benchmark breadth | Deterministic quality canaries and answer-eval reporting exist, but not a broader scheduled benchmark with historical trend reporting. | Partial | Eval harness covers current facts, official docs, local/weather, shopping, adversarial pages, insufficient-evidence refusal, answer-quality score regressions, latency, zero-spend cost posture, and citation precision. |
...
| 5 | Eval harness v1 | Partial | Scheduled canary status is surfaced through Beacon health and Helm summary; answer-quality scenarios cover strong vendor comparisons, missing official coverage, unsupported pricing refusal, and prompt-injection refusal; eval payload reports latency, zero-spend cost posture, planned budgets, and citation precision. Historical trend reporting remains. |
...
3. Eval harness breadth: scheduled historical trend reporting for latency, cost, and citation precision.
